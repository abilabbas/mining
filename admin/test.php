...
if($result = mysqli_query($conn,'SELECT layanan.layanan_name, produk.produk_name 
FROM transaksi 
JOIN (
  SELECT 
   layanan.id_layanan,
   GROUP_CONCAT(layanan.layanan_name SEPARATOR ",") AS layanan_name
  FROM layanan
  GROUP BY layanan.id_layanan
) layanan ON transaksi.id_layanan = layanan.id_layanan
JOIN
(
  SELECT 
   produk.id_produk,
   GROUP_CONCAT(produk.produk_name SEPARATOR ",") AS produk_name
  FROM produk
